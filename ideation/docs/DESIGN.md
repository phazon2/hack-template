...
class IdeationState:
    ...
    ideas: list[Idea] = []
    angles: list[str] = []              # chain-of-thought: the angles set before each batch
    verdicts: list[PanelVerdict] = []
    ranking: list[str] = []
    pairwise_wins: dict[str, int] = {}  # idea_id -> wins against the reference winners
...
- `rank_ideas(ideas, verdicts, pairwise: dict[str, int] | None = None) -> list[str]`: tier 1 non-disqualified with consensus feasibility > 2; tier 2 non-disqualified with feasibility <= 2; tier 3 disqualified — a permutation of all idea ids. Ideas without a feasibility criterion count as feasibility 3. Sort key `(tier, -pairwise_wins, -weighted_score, title, id)`: the panel sets the tier, pairwise wins order the tier, weighted score only breaks ties. With `pairwise` empty every idea has 0 wins, so the order falls back to `(-weighted_score, title)` — the pre-pairwise behaviour.
- `compare_ideas(panel, ideas, context, pairwise=None) -> tuple[list[PanelVerdict], list[str]]`: `panel.evaluate_many` then `rank_ideas`.

### 8.4 Pairwise ranking (`evaluation/pairwise.py`)
Direct rubric scoring by an LLM ranks ideas at roughly chance (Si et al., arXiv 2409.04109: GPT-4o
50.0, Claude-3.5 51.7, where random is 50.0 and humans reach 66-72). The only shape in that work
that beat chance was pairwise comparison against references with known outcomes. So the panel sets
tiers and pairwise sets the order inside them.
- `winner_references(texts: list[str], limit=6) -> list[str]`: lines matching `- Name: tagline`
  under a `WINNERS` heading, stopping at `LISTED WITHOUT A WINNER LABEL` or any other all-caps
  heading; document order, deduplicated, capped at `limit`.
- `references_from_corpus(settings, limit=6) -> list[str]`: `load_corpus(settings.all_corpus_dirs())`,
  keep documents with `kind == "evidence"`, then `winner_references`. Reads WHOLE documents, never
  retrieved chunks: chunking splits a winners block and returns whichever fragment matched the
  query. Returns `[]` on `OSError`.
- `pairwise_scores(ideas, references, llm, max_tokens, effort) -> dict[str, int]`: one batched call,
  tag `pairwise`, schema `obj({"comparisons": arr(obj({"idea_id": enum(ids), "beats": int_(0, len(references)), "hardest": str_()}), len(ids), len(ids))})`.
  Returns `{}` when fewer than 2 ideas, no references, on `LLMError`, or when the response does not
  cover every id — an empty result means the caller falls back to the panel's own ordering rather
  than producing a fake rank.
...
- Canonical tags (`LLMRequest.tag` == `TraceStep.agent`): `strategist`, `orchestrator`, `research`, `domain_expert`, `creativity`, `dedupe`, `judge:<persona-slug>`, `pairwise`, `rerank`, `synthesizer`, `reflector`, `learn`, `probe`.
...
`orchestrator -> research -> retrieve_more -> (cond A) -> domain_expert -> creativity (-> dedupe) -> evaluator (-> pairwise) -> (cond B) -> synthesizer -> END`
...
- **CreativityAgent** (`creativity`): increments `state.iteration` on entry; ids `idea-{iteration}-{n}`; APPENDS to `state.ideas`. Schema `obj({"distinct_angles": arr(str_(), n, n), "ideas": arr(IDEA_SCHEMA, n, n)})`, n = `settings.ideas_per_round`; `distinct_angles` comes FIRST so the model reasons before it answers (structured output fills properties in order) — chain-of-thought was the highest-diversity strategy of the 35 in Meincke et al. (arXiv 2402.01727), and diversity is the measured bottleneck. Angles are appended to `state.angles`; `IDEA_SCHEMA` fields: title, one_liner, description, target_user, key_innovation, `technique: enum(TECHNIQUES)`, technical_approach, demo_strategy, demo_moment, `data_sources: arr(str_(), 1, 4)`, `mvp_scope: arr(str_(), 1, 5)`, `cut_first: arr(str_(), 1, 4)`, closest_existing, `build_hours_estimate: int_(1, constraints.hours)`, `risks: arr(str_(), 1, 4)`, `citations: arr(enum(ids), 0, len(ids))`, plus `parent_id: enum(top3_ids + ["new"])` in round >= 2 (`"new"` -> `None`). Prompt: cycle the techniques (idea k uses `TECHNIQUES[k % 6]`), constraints_block, memory_context (leverage/avoid), `building_blocks` as the resource menu, antipattern chunks as "do not produce unless you state the twist", require >= 4 distinct target_users and >= 3 distinct primary data_sources across the batch, each idea cites >= 1 shown chunk; ideas using anything in Must avoid are invalid. Before the ideas, fill `distinct_angles` with exactly n entries in slot order, each naming the person-in-situation and the angle, differing on user and mechanism rather than wording. When `references_from_corpus` yields winner taglines they are shown as few-shot examples of SHAPE ONLY, with an explicit ban on reusing their subject matter, domains, product names or phrasing (Girotra et al.: highly rated examples improve output; without the ban they leak their domains into the batch). Round >= 2: also the top-3 ideas (title, one_liner, consensus weaknesses/suggestions) and `state.critiques`; roughly half refine a top-3 idea (`parent_id`, must state what changed) and half are new.
  Deduplication runs in two stages after validation and before ids are assigned (survivors carry provisional `cand-{iteration}-{k}` ids so stage two can name them). Stage one, `diverse(candidates, earlier, threshold=0.65)`: cosine over `HashingEmbedder(dim=512)` fitted on the batch, dropping a candidate too close to any earlier or already-kept idea. A candidate whose `parent_id` names an earlier idea is EXEMPT from that one parent only — a refinement is meant to resemble what it refines, and without the exemption the filter keeps the original and deletes the improvement. Stage two, `drop_semantic_duplicates` (`agents/dedupe.py`, tag `dedupe`, effort light, >= 3 ideas): the model groups ideas that are the same idea reworded; the first of each group is kept. On `LLMError` it returns no groups, so a failure never loses ideas. Both stages add to `state.dropped_duplicate`.
...
- **EvaluatorAgent** (`evaluator`): `PanelJudge(ctx.llm, ctx.rubric, personas=[p.format_map(Defaulting(hours, team_size, theme)) for p in settings.judge_personas])`; judges only ideas without a verdict (verdicts keyed by `idea_id`); context = constraints_block + assessment (challenges, building_blocks, hour_budget) + top research bullets; recomputes `state.ranking` over ALL ideas via `rank_ideas`. When there are ideas to judge it also recomputes `state.pairwise_wins = pairwise_scores(state.ideas, references_from_corpus(ctx.settings), ...)` (tag `pairwise`, effort light, skipped when there are no references); with nothing pending it re-ranks from the CACHED wins and makes no call, because recomputing would pay for a comparison whose inputs have not changed and dropping them would silently reorder. replaces `state.critiques` with, for each top-3 idea, its consensus weaknesses plus `"lowest criterion: {name}"`, deduped, <= 9 entries.
...
dependencies; (5) Runner-ups with "choose instead if …"; (5a) "Angles the batch set out to cover", only when `result.angles` is non-empty (`ANGLES_HEADING`); (6) All-ideas table (title, technique,
beats, weighted, feasibility, agreement, disqualified) in ranking order, where "beats" is the
pairwise win count or "-" when the idea was not compared; (7) Per-idea detail;
