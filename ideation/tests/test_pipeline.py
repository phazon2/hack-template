...
from ideate.models import HackathonConstraints, Idea, IdeationResult, Outcome, Pattern
from ideate.pipeline import IdeationSystem, receipt_stamp, result_from_state
...
def test_e2e_default_mock(e2e):
    ...
    assert len(result.trace) == 1 + result.retrieval_rounds + 1 + result.iterations * (1 + len(settings.judge_personas)) + 1
...
def test_ideate_with_reranker_none_still_completes(settings):
    ...
    assert settings.ideas_per_round <= len(result.ideas) <= settings.ideas_per_round * result.iterations
